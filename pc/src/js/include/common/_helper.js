...
export function popupOpen(item) {
  ...
  const oTargetPopup = document.querySelector(`.popup[data-popup=${oPopupName}]`);
  const isFull = oTargetPopup.getAttribute('data-popup-type') === 'full' ? true : false;
  ...
  document.querySelector(`.popup[data-popup=${oPopupName}]`).classList.add(`${ACTIVE}`);
}
...
export function popupClose(e) {
  ...
  const oDim = document.querySelector('.popup-dim');
  ...
  if (isDim) {
    ...
    const oTargetPopup = document.querySelector(`.popup[data-popup=${sPopupDimAttr}]`);

    oTargetPopup.classList.remove(`${ACTIVE}`);
    oDim.classList.remove(`${ACTIVE}`);
  } else {
    // 딤드가 활성화 됐다면
    oDim.classList.contains(`${ACTIVE}`)
    && oDim.classList.remove(`${ACTIVE}`);

    const oClosestPopup = oTarget.closest('.popup');
...
export function toggleClass(oTarget) {
  ...
  const oToggleClass = oTarget.getAttribute('data-toggle').trim();
