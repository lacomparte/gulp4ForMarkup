...
module.exports = gulp.task(config.taskname, function () {
    ...
    gulp.watch(config.html, gulp.series(['html-watch']));
    gulp.watch(config.sass, gulp.series(['sass-watch'])).on('change', clearError);
    gulp.watch(config.javascript, gulp.series(['javascript-watch']));
    gulp.watch(config.image, gulp.series(['copyimg']));
    gulp.watch(config.__image, gulp.series(['copyim', 'imgsize']));
    gulp.watch(config.sprite, gulp.series(['sprite']));
});
